...
function formatPopup(busStop) {
    let iframe = "";
    if (busStop.informations.id != null)
        iframe = `<iframe src='http://www.swtue.de/abfahrt/?halt=${busStop.informations.id}'></iframe>`;
    let table = "<table>";
    table += Object.keys(filters).map(k => formatTableRowInPopup(busStop, k)).join("");
    if (busStop.informations.operator != null)
        table += `<tr><td>Betreiber</td><td>${busStop.informations.operator}</td></tr>`;
    if (busStop.informations.lines != null)
        table += `<tr><td>Linien</td><td>${formatMultivalue(busStop.informations.lines)}</td></tr>`;
    table += "</table>";
...
function formatTableRowInPopup(busStop, key) {
...
function busStopFulfilsFilter(busStop, filter) {
